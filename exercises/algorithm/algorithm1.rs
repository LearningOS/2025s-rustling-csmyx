...
impl<T> LinkedList<T> {
        ...
        T: PartialOrd,
    {
        ...
        while cur_a.is_some() || cur_b.is_some() {
            if cur_b.is_none()
                || (cur_a.is_some()
                    && unsafe { (*cur_a.unwrap().as_ptr()).val < (*cur_b.unwrap().as_ptr()).val })
            {
                cur = cur_a;
                cur_a = unsafe { (*cur_a.unwrap().as_ptr()).next };
            } else {
                cur = cur_b;
                cur_b = unsafe { (*cur_b.unwrap().as_ptr()).next };
            }
            if list.start.is_none() {
                list.start = cur;
            }
            if let Some(x) = list.end {
                unsafe { (*x.as_ptr()).next = cur };
            }
            list.end = cur;
        }
